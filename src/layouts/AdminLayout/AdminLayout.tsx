...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans text-gray-900 dark:bg-gray-900 dark:text-gray-100">
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-white shadow-xl dark:border-r dark:border-gray-800">
        {/* Logo */}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {NAV_SECTIONS.map((section) => (
            <div key={section.section} className="mb-4">
              <p className="mb-1 px-3 text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">
                {section.section}
              </p>
        ...
        <div className="border-t border-gray-100 p-3">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700/50 dark:hover:text-gray-100"
          >
      ...
      <main className="ml-64 flex min-h-screen flex-1 flex-col">
        {/* Topbar */}
        <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-end border-b border-gray-100 bg-white/80 px-8 shadow-sm backdrop-blur-md dark:border-gray-800 dark:bg-gray-900/80">
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-sm font-semibold text-gray-800 dark:text-gray-200">{userProfile?.name || "Admin"}</div>
              <div className="text-xs text-gray-400 dark:text-gray-500">{userProfile?.email}</div>
            </div>
